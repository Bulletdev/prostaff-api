...
if [ -n "$DB_URL" ]; then
  ...
  until pg_isready -h "$DB_HOST" -p "$DB_PORT" > /dev/null 2>&1; do
    ...
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
      echo "  [ERROR] Database connection timeout after ${MAX_RETRIES} attempts" >&2
      exit 1
    fi
    echo "  [WAIT] Waiting for database (attempt ${RETRY_COUNT}/${MAX_RETRIES})..." >&2
    sleep 2
  done
  echo "  [OK] Database is ready" >&2
else
  echo "  [WARNING] No DATABASE_URL configured, skipping database check" >&2
fi
...
if [ -n "$REDIS_URL" ]; then
  ...
  if timeout 5 bash -c "echo > /dev/tcp/${REDIS_HOST}/${REDIS_PORT}" 2>/dev/null; then
    echo "  [OK] Redis is reachable" >&2
  else
    echo "  [WARNING] Redis connection failed - Sidekiq will not work properly" >&2
    echo "  → Hostname resolution issue or Redis not accessible" >&2
    echo "  → Attempting DNS resolution for ${REDIS_HOST}..." >&2
    if command -v host > /dev/null 2>&1; then
      host "$REDIS_HOST" >&2 || echo "  [ERROR] DNS resolution failed" >&2
    elif command -v nslookup > /dev/null 2>&1; then
      nslookup "$REDIS_HOST" >&2 || echo "  [ERROR] DNS resolution failed" >&2
    else
      echo "  → No DNS tools available to diagnose" >&2
    fi
  fi
else
  echo "  [WARNING] No REDIS_URL configured, Sidekiq will run in inline mode" >&2
fi
