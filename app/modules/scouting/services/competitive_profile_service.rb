      ...
      def format_response(raw, pro_name)
        {
          ...
          competitive_games: raw['total_games'] || raw[:total_games] || 0,
          competitive_win_rate: raw['win_rate'] || raw[:win_rate] || 0.0,
          leagues_played: raw['leagues'] || raw[:leagues] || [],
          champion_pool_competitive: format_pool(raw),
          avg_kda_competitive: raw['avg_kda'] || raw[:avg_kda],
          last_competitive_game: raw['last_game'] || raw[:last_game]
        }
      end

      def format_pool(raw)
        pool = raw['champion_pool'] || raw[:champion_pool] || []
        pool.map { |c| format_pool_entry(c) }
      end

      def format_pool_entry(entry)
        {
          champion: entry['champion'] || entry[:champion],
          games: (entry['games'] || entry[:games] || 0).to_i,
          wins: (entry['wins'] || entry[:wins] || 0).to_i,
          win_rate: (entry['win_rate'] || entry[:win_rate] || 0.0).to_f,
          avg_kda: entry['avg_kda'] || entry[:avg_kda]
        }
      end
end
