...
module MetaIntelligence
  module Jobs
    ...
    #   MetaIntelligence::Jobs::UpdateMetaStatsJob.perform_later(org.id)
    #
    # @example Enqueue for a specific patch
    #   MetaIntelligence::Jobs::UpdateMetaStatsJob.perform_later(org.id, patch: '14.24')
    class UpdateMetaStatsJob < ApplicationJob
      ...
      def run_aggregation(organization, scope, patch)
        Services::BuildAggregatorService.new(
      ...
      def sync_to_search(organization)
        indexer = Services::MetaIndexerService.new(organization: organization)
        indexer.sync_builds
        indexer.sync_items
      end
