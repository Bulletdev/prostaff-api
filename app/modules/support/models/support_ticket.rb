...
class SupportTicket < ApplicationRecord
  ...
  scope :by_priority, lambda {
    order(Arel.sql("CASE priority WHEN 'urgent' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END"))
  }
  ...
  def track_status_changes
    ...
    update_column(:first_response_at, Time.current) if first_response_at.nil? && new_status == 'in_progress'

    # Track resolution
    update_column(:resolved_at, Time.current) if resolved_at.nil? && new_status == 'resolved'

    # Track closure
    return unless closed_at.nil? && new_status == 'closed'
