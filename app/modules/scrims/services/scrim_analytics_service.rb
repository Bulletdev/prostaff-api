...
class ScrimAnalyticsService
  ...
  def stats_by_opponent
    ...
    scrims.group_by(&:opponent_team_id).filter_map do |opponent_id, opponent_scrims|
      next unless opponent_id

      # Use the already-preloaded opponent_team from the first scrim
      # instead of firing a separate query per opponent (N+1)
      opponent_team = opponent_scrims.first.opponent_team
