    ...
    module Strategy
      ...
      class TacticalBoardsController < Api::V1::BaseController
        ...
        def tactical_board_params
          # Get the tactical_board params
          board_params = params.require(:tactical_board)

          # Extract permitted scalar fields
          permitted = board_params.permit(
            :title,
            :match_id,
            :scrim_id,
            :game_time
          ...
          # Add JSON fields (these don't go through strong params)
          permitted[:map_state] = board_params[:map_state].as_json if board_params[:map_state].present?
          permitted[:annotations] = board_params[:annotations].as_json if board_params[:annotations].present?
